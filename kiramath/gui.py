...
class Calc():
    ...
    def do_sum(self):
        ...
        if self.op == "cos":
            self.total = elem.cos(self.current)
        if self.op == "exp":
            self.total = elem.exp(self.current)
        if self.op == "log":
            self.total = elem.log(self.current)
        if self.op == "fac":
            self.total = elem.fac(self.current)
        if self.op == "sin":
            self.total = elem.sin(self.current)
        if self.op == "sqrt":
            self.total = elem.sqrt(self.current)
        if self.op == "tan":
            self.total = elem.tan(self.current)
...
exp = Button(calc, text = "exp")
exp["command"] = lambda: sum1.operation("exp")
exp.grid(row = 1, column = 4, pady = 5)

log = Button(calc, text = "log")
log["command"] = lambda: sum1.operation("log")
log.grid(row = 2, column = 4, pady = 5)

sqrt = Button(calc, text = "sqrt")
sqrt["command"] = lambda: sum1.operation("sqrt")
sqrt.grid(row = 3, column = 4, pady = 5)

sin = Button(calc, text = "sin")
sin["command"] = lambda: sum1.operation("sin")
sin.grid(row = 1, column = 5, pady = 5)

cos = Button(calc, text = "cos")
cos["command"] = lambda: sum1.operation("cos")
cos.grid(row = 2, column = 5, pady = 5)

tan = Button(calc, text = "tan")
tan["command"] = lambda: sum1.operation("tan")
tan.grid(row = 3, column = 5, pady = 5)

fac = Button(calc, text = "!")
fac["command"] = lambda: sum1.operation("fac")
fac.grid(row = 4, column = 2, pady = 5)
